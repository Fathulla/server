...
router.post(
  "/",
  upload.single("img"),  // Используем middleware для загрузки одного файла
...
router.get("/", async (_req, res) => {
  try {
    const events = await PrivateEvent.findAll();
    res.json(events);
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    const event = await PrivateEvent.findByPk(req.params.id);
    if (event) {
      res.json(event);
    } else {
...
router.delete("/:id", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const deleted = await PrivateEvent.destroy({
      where: { id: req.params.id },
    });
